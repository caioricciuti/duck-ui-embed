...
import type { QueryResult, ColumnInfo } from './result'
...
/**
 * Coerce DuckDB-WASM Arrow types to plain JS primitives.
 * Handles BigInt, Decimal objects, Date objects, etc.
 */
function coerceValue(value: unknown): unknown {
  if (value === null || value === undefined) return value

  // BigInt → Number (safe for most analytics values)
  ...
  if (typeof value === 'object' && value !== null) {
    ...
    // Arrow Decimal/Int types with low/high fields
    ...
    // Arrays (LIST columns) — coerce each element

...
export interface UChartProps extends ChartOptions {
  data: ChartData['data']
  labels?: string[]
  /** Category labels for the x-axis (when x values are categorical strings) */
  xLabels?: string[]
  className?: string
  /** Chart theme */
  theme?: ChartTheme
...
export function UChart({
  ...
  const buildOptions = useCallback((): uPlot.Options => {
    ...
    // Build series with theme-aware colors
    ...
    const series: uPlot.Series[] = [
      ...
      ...Array.from({ length: seriesCount }, (_, i) => {
        ...
        if (isBar) {
          // Bar series: fill with color, no stroke line
          return {
            label: labels?.[i + 1] ?? `Series ${i + 1}`,
            stroke: color,
            width: 0,
            fill: color + 'cc', // slightly transparent fill
    ...
    if (tooltip) {
      ...
      // When categorical, use xLabels for tooltip x display
    ...
    // Disable bottom legend when tooltip is enabled (tooltip replaces it)
    const showLegend = tooltip ? false : legend

    // Build axes with theme styling
    ...
    if (isCategorical) {
      ...
      scales.x = {
        ...scales.x,
        time: false,
        distr: 2, // evenly spaced discrete
